...
do
	...
	run_cmd "bowtie2 --very-sensitive-local --no-unal -x $REF -1 $R1 -2 $R2 --threads $NCPUS 2>$OUT.bowtie2_local_mapping.stats | samtools sort -@ $NCPUS - > $OUT.local_mapping.coordsorted.bam"
done < samples.txt
