...
while read PEP; do
	...
	# Filter blast output. Filter by e-value < 1e-5 (e-values upto 1000 was allowed during blast analysis).
	...
	cat "$PEP.blastp_"{BACTERIA,METAZOA,MMETSP,OTHER}".outfmt6" | awk -F'\t' '($11 < 1e-5)' | sort -k1,1 -k12,12nr > "$PEP.blastp_ALL.outfmt6"
