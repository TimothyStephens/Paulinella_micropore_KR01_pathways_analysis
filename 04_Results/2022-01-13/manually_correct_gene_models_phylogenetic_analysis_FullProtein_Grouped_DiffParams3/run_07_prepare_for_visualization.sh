...
while read KO; do
	...
	echo "## $ALN"
		...
		| ./add_value_to_table.py -a <(awk -F'\t' 'BEGIN{print "SeqName\tcrTP\tmtTP"} $1!="seqid" {print $0}' ../../2021-01-19/organelle_targeting_peptides/*.organelle_TP.txt *.organelle_TP.txt) -d $'No\tNo' \
		> "${ALN}.annots.txt"
       	else
